...
print(df['dien_tich'].notnull().sum()/df['dien_tich'].shape[0])
...
def convert_furniture(furniture):
    try:
        ...
        if _found_text(furniture, ['kéo vali', 'xách vali', 'mang vali', 'ở ngay', 'full đồ', 'mang quần áo vào ở', 'đủ đồ']):
            return 2
        elif _found_text(furniture, ['đồ cơ bản', 'nguyên bản', 'đã có đồ']):
            return 1
        elif (_found_text(furniture, ['điều hoà', 'điều hòa', 'tủ bếp'])):
